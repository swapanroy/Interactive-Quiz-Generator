...
const App: React.FC = () => {
  ...
  const handleGenerateQuiz = async (generatedFiles: MediaFile[], difficulty: 'easy' | 'medium' | 'hard') => {
    try {
      setIsLoading(true);
      // We update files state with the specific files used for generation (important for PDF extraction)
      setFiles(generatedFiles); 
      const data = await generateQuizFromMedia(generatedFiles, difficulty);
      setQuizData(data);
    ...
    <>
      <div className="min-h-screen bg-slate-50 relative overflow-x-hidden text-slate-900 font-sans">
        ...
        <footer className="py-8 text-center text-slate-400 text-sm font-medium relative z-10">
          ...
          <p className="opacity-60 mb-2">&copy; {new Date().getFullYear()} Quiz Me</p>
          <p className="text-xs text-slate-500">Created by <span className="font-bold text-slate-400">Swapan Roy</span></p>
        </footer>
      ...
      {/* Voice Tutor Overlay - Placed outside the main container to avoid potential overflow issues */}
      <VoiceTutor files={files} />
    </>
  );
};
